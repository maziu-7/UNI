...
// Descomenteu les seg�ents dues linies i implementeu el m�tode:
  Arbre suma (const Arbre& a) const{
    Arbre res;
    res.primer_node = sum_aux(primer_node, a.primer_node);
    return res;
  }
private:
  
  static node_arbre* sum_aux(node_arbre* n,  node_arbre *m){
    if (n == NULL or m == NULL) return NULL;
    else {
      node_arbre* p = new node_arbre;
      p->info = n->info + m->info;
      p->segE = sum_aux(n->segE, m->segE);
      p->segD = sum_aux(n->segD, m->segD);
      return p;
    }
  }  
