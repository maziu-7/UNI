...
void evaluate(BinTree<string> t, int &result, bool &error) {
    if (isNumber(t.value())) {
        result = mystoi(t.value());
        error = false;
    }
    else {
        int result_left = 0;
        evaluate(t.left(),result_left,error);
        if (not error) {
            int result_right = 0;
            evaluate(t.right(),result_right,error);

            if (t.value() == "+") result = result_left+result_right;
            else if (t.value() == "-") result = result_left-result_right;
            else if (t.value() == "*") result = result_left*result_right;
            else if (t.value() == "/" and result_right != 0) result = result_left/result_right;
            else error = true;
        }
    }
}