...
public:
  ...
  class iterator {
    ...
    private:
      ...
      int jump_plus = 1;
      int jump_less = 1;
      // Add attributes to remember how the iterator should move depending on calls
      // to changePlusMove and changeMinusMove.
      // ...
      
      // Add new private functions if you wish.
      
    public:
    
    iterator() {
        // Add initialisation of new attributes.
        // ...
    }
      
	// Adapt this function to new attributes, and so that an attempt to
    // go out of range does not trigger error, but leaves the iterator unchanged.
    // Preincrement
    iterator operator++()
    ...
    {
      if(jump_plus > 0) {
        for(int i = 0; i < jump_plus and (pitem != &(plist->itemsup)); i++) {
          pitem = pitem->next;
        }
      }
      else if (jump_plus < 0) {
        for(int i = jump_plus; i < 0 and (pitem != plist->iteminf.next); i++) {
          pitem = pitem->prev;
        }
      }
      return *this;
    ...
    {
      if(jump_less > 0) {
        for(int i = 0; i < jump_less and (pitem != plist->iteminf.next); i++) {
          pitem = pitem->prev;
        }
      }
      else if (jump_less < 0) {
        for(int i = jump_less; i < 0 and (pitem != &(plist->itemsup)); i++) {
          pitem = pitem->next;
        }
      }
      return *this;
