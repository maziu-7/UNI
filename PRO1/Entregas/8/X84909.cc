...
void printSlotTeacher(vector <string> slotTeachers, int& replacementsNeeded) {
    ...
    for (int x = 1; x < slotTeachers.size(); ++x) {
        if (slotTeachers[x] == slotTeachers[x - 1]) ++replacementsNeeded;
        ++repeatedTeachers;
    }
    int uniqueTeachers = slotTeachers.size() - repeatedTeachers;
    int spaceCounter = 9;
    while (uniqueTeachers / 10 > 0) {
        uniqueTeachers /= 10;
        --spaceCounter;
    }
    for (int x = 0; x < spaceCounter; ++x) cout << ' ';
